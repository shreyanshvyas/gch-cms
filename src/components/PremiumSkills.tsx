// src/components/PremiumSkills.tsx
import { cn } from "./cn";
...
export type SkillCard = {
  ...
  percent: string;
  description: string;
  color: "magenta" | "indigo" | "violet" | "green" | "amber";
};

export type PremiumSkillsProps = {
  heading: string;
  subheading?: string;
  items: SkillCard[]; // order: 0..4 (top 3, then bottom 2)
};

const colorMap: Record<SkillCard["color"], string> = {
  magenta: "bg-[#9E165F]",   // adjust to match brand
  indigo: "bg-[#2D31A6]",
  violet: "bg-[#5720B7]",
  green: "bg-[#335015]",
  amber: "bg-[#854A0E]",
...
export default function PremiumSkills({ heading, subheading, items }: PremiumSkillsProps) {
  return (
    <section className="bg-[#E3EDF9] relative">
      <div
        aria-hidden
      ...
      <div className="mx-auto max-w-7xl px-6 py-12 sm:py-20">
        <div className="mb-10 text-center">
          ...
          {subheading ? (
            <p className="mx-auto max-w-3xl text-base leading-7 text-gray-600">{subheading}</p>
          ) : null}
        </div>

        {/* Responsive grid: 1 -> 2 -> 3 columns, then explicit 12-col layout */}
        <div className="grid gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-12">
          {items.map((card, i) => {
            // Desktop placement: first 3 take 4 cols each; last 2 centered as 5+5
            const lgPos =
              i === 0 || i === 1 || i === 2
                ? "lg:col-span-4"
                : i === 3
            ...
            return (
              <article
                key={i}
                className={cn(
                  "h-full rounded-[28px] md:p-10 p-6 text-white shadow-sm", // big radius pill
                  "flex flex-col justify-between",                  // equal heights
                  colorMap[card.color],
                  lgPos
                )}
              >
                <div>
                  <h3 className="mb-4 md:text-2xl text-xl font-semibold leading-tight">{card.title}</h3>
                  <div className="flex md:flex-row flex-col md:items-center gap-3">
                    <h2 className="md:text-6xl text-5xl font-extrabold leading-none">{card.percent}</h2>
                    <p className="text-base/7 opacity-90">{card.description}</p>
                  </div>
                </div>
              </article>
            );
